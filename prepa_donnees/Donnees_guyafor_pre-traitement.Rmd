...
fich_sauv_circ<-"Data_guyafor_mch_join"  # nom du fichier de sortie
...
Pour une espèce on ne conserve un site seulement s'il y a assez d'arbres. Pour rappel, le seuil se gère avec le paramètre "nbmin_arbres" 'par défaut nbmin_arbres=0
...
Dmaxnbval_site<-Dmax_esp_site %>% 
  group_by(idEsp) %>% 
  summarise(nbval=n_distinct(Dmax95))
  
  ggplot(Dmax_esp_site, aes(x=idEsp,y=Dmax95))+
    geom_boxplot()+
    xlab("")+ylab("Diamètre max")+
    theme_bw() +
    # theme(axis.text.x = element_text(size=14, angle=90,hjust = 1,vjust=0.25))
    theme(axis.text.x = element_text(size=14, angle=60,hjust=1,vjust=1))+
    geom_text(data=Dmaxnbval_site,aes( y=10, label=nbval))+
    ylim(min=0,130)
...
datamo_sc<-tabMesuresSelect%>%
  arrange(idTree,CensusYear)%>%   # on trie par arbre et annee
  select(idTree,idEsp,Diam,CodeAlive,CodeMeas,CensusYear,CensusDate,Dmax95,Forest,Plot,idTraitement)%>%  # choix de quelques colonnes
  ...
  mutate(Diam2=lead(Diam),CodeAlive3=lead(CodeAlive,2),CodeMeas3=lead(CodeMeas,2),idTree3=lead(idTree,2),Year2=lead(CensusYear),Year3=lead(CensusYear,2),CDate2=lead(CensusDate),CDate3=lead(CensusDate,2))%>% 
  filter(idTree==idTree3)%>%    # on enleve les lignes qui avec des donnees de deux individus differents
  select(idTree,idEsp,Diam1=Diam,Diam2,
         CodeAlive3,CodeMeas3,Year1=CensusYear,Year2,Year3,CDate1=CensusDate,CDate2,CDate3,Dmax95,Forest,Plot,idLogg=idTraitement)%>% # quelques nouveaux titres de colonne"
...
#1 Chargement des donnée REW par jour et par site (donnees journalières de REW avec date en trois colonnes : year, month, day) ####
...
# tableau min_max par site
dispoMinMaxIshDate<-ISHdata %>% 
  group_by(Forest) %>% 
  summarise(MinCDclim=min(CDymIsh),MaxCDclim=max(CDymIsh))


#2 Construction d'un tableau des sites et couples d'inventaire à partir des mesures sélectionnées ####
...
#3 Boucle de calcul de l'indice pour chaque ligne foret_couple_inventaire ####
...
#4 Graphe indices REW journaliers ####
...
clim_site<-REW_sites_gfclim %>%  # REW_sites_gfclim est issu du ficher Donnees_guyafor_pre-traitement.Rmd
  ...
  filter(Forest=="Montagne Tortue") %>% # choix du site
  filter(year>=2010&year<2019) # choix des années

ggplot(clim_site)+
  geom_line(aes(x=day_julian, y=REW), color="blue")+
  facet_wrap(~year,scales="free_x",nrow=3)+ # fixe le nombre de ligne de graphes
  geom_hline(yintercept = 0.4, linetype = "dashed")+
  geom_ribbon(aes(x=day_julian,ymin=REW04, ymax=0.4,fill="grey"),color="blue")+
  xlab("site de Montagne Tortue (Régina)") + # toitre du graphe sur l'axe des X
  theme(axis.title.x = element_text(size=15,hjust=0.5,vjust=0))+
...
ggplot(clim_site)+
  ...
  geom_hline(yintercept = 0.4, linetype = "dashed")+
  geom_ribbon(aes(x=day_julian,ymin=REW04, ymax=0.4,fill="grey"),color="blue")+
  xlab("site d'Organabo (Mana)") + # toitre du graphe sur l'axe des X
  theme(axis.title.x = element_text(size=15,hjust=0.5,vjust=0))+
  theme(legend.position = "none")+ # pour ne pas faire apparaitre la légende
  scale_x_continuous(breaks=NULL,labels=NULL)
...
index_trait<-index_trait %>% 
  left_join(select(DispoLogg,ForestPlot,NomForet,Libelle,Post_CY,Pre_CY),by="ForestPlot") %>% # on récupere les donnée année pre et post logg par parcele exploitée
  rename(Forest=NomForet,Plot=Libelle)
```
...
# Data_guyafor_16esp_PPlogg_join pour ajout annee pre et post Logg sur indextrait
...
nb_data_site<-dispoGuyafor %>% 
  group_by(Forest,Plot,CensusYear) %>% 
  summarise(nbmes=n()) %>%
  group_by(Forest) %>% 
  summarise(MeanPlotmes=mean(nbmes))
  
  
REW_pc_forest<-REW_sites_gfclim %>% 
  group_by(Forest) %>% 
  summarise(REWpc1=quantile(REW,probs=0.01),
            REWpc5=quantile(REW,probs=0.05),
            REWmediane=median(REW),
            REWpc95=quantile(REW,probs=0.95),
            REWpc99=quantile(REW,probs=0.99)) %>% 
  arrange(REWpc5)
