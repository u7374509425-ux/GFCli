...
fich_sauv_circ<-"Data_guyafor_16esp_Cl_join"  # nom du fichier de sortie
...
#2 Création de l'indice de climat : stress hydrique (à faire : pb de certains inventaires on a NA dans climmo. Ils sont pas représentés dans climvig)
...
for(i in 1:nrow(ForestInvIsh)) {
 # ISH(ParacouInvIsh$CDym1[i],ParacouInvIsh$CDym2[i],ParacouInvIsh$Forest[i],ISHdata)
  ...
  ForestInvIsh$IshInv[i]<-sum(sortie$Ishm)
  }
...
save(datacr_cl,datamo_cl,file="data_WaterStress_guyafor.rdata") # le 17/01/2020
...
## Choix des années d'inventaire pré et post exploitation quand elles existent
...
save(nbmin_arbres,            # nombre minimal d'arbres par esp et par site
     ...
     file=paste(fich_sauv_circ,".Rdata",sep="")) #Data_guyafor_16esp_Cl_join.Rdata fichier avec donnée climat multisite
