...
```{r nettoyage, echo=FALSE, message=FALSE}
# on commence par faire le menage
rm(list=ls(all=TRUE))
gc() # garbage collector
```

```{r Bibliotheque, echo=FALSE, message=FALSE}
...
# Résumé
Mise au format des données Guyafor pour modele joint croissance mortalité à faire tourner sous Stan. Les données de base sont issues d'une extraction de Guyafor : DataParacouV6.R : cree dispoGuyafor 1887076 obs et 31 variables.



#1 Création données arbres, calcul Dmax par site et par espece depuis Guyafor 
Le fichier **Data_guyafor_16esp_to_join.Rdata** contient le résultat de ce script, paramètres (nbmin_arbres=20 ; nbmin_mes=3 ; an_min_meteo=1978)
...
an_min_meteo<- 1978  # donnee meteo les plus anciennes
fich_sauv_circ<-"Data16esp_s0_to_join.Rdata"  # nom du fichier de sortie
...
## Chargement des données
 ...
 2. Chargement des données sur le type de traitement par placete et par site
...
## Construction de tableau pour la selection des arbres et de leur mesures
...
```
Au final il reste 248837 Mesures

###2 Creation d'une liste des arbres selectionnés
On supprime les arbres à problème : par exemple arbres avec hauteur de mesure modifiée. On perd presque 1/5 des donnees.
Puis on ne conserve que les arbres avec au moins 3 inventaires. Au final on a 22 545 arbres
...
```{r tableau_eff,echo=FALSE}
#knitr::kable(tab_esp, caption="Effectif d'arbres et de mesure par espèce et traitement",align=c('r','c','r','c','c','r','r'))
```
##Conversion des circonférence en diamètre et régularisation pour les nouveaux morts
...
Dmax_esp<-tabMesuresSelect %>%           
  ...
  group_by(idEsp,codeEsp) %>%
  summarise(nbtree=n(),Dmax95_esp=quantile(DmaxTree,0.95,na.rm=TRUE)) %>%  
...
Dmax_esp_site<-tabMesuresSelect %>%           
  ...
  summarise(nbtree=n(),Dmax95=quantile(DmaxTree,0.95,na.rm=TRUE)) %>%  
  select(idEsp,codeEsp,Forest,Dmax95,nbtree) %>% #mise en forme
  arrange(idEsp,Dmax95) %>% 
  left_join(Dmax_esp,by=c("idEsp")) %>% 
  mutate(Dmax95 = if_else(nbtree >= seuil, Dmax95, Dmax95_esp)) %>% 
  select(-Dmax95_esp)
...
#2 Création de l'indice de climat : stress hydrique ( afaire : pb decertain inventaire on a NA dans climmo. Ils sont pas représentés dans climvig)
...
Il est alors possible de construire un indice de stress hydrique sur des durées supra-journalière (semaine, mois, années) en sommant, uniquement pour les journées avec un REW inférieur à 0.4, les écart entre le REW observé et le seuil de 0.4. Sur un graphique montrant l'évolution journalière du REW, cet indice correspond à l'aire "vide" sous la droite REW=0.4 (cf graphe ci-dessous) (cit Aubry-Kientz et al 2015)

Pour cette étude nous avons à notre disposition des données sols pour le site de Paracou et des données pluviométriques pour la pluspart des sites Guyafor. Nous allons donc construire de REW journalier pour chaque site Guyafor avec sa pluviométrie, mais en conservant les données sol de Paracou.

Seules les donnéee REW pour le dispositifs de Paracou sont disponible

La construction de la covariable climat se fait en deux étapes

1. calcul d'un indice de stress interinventaire par site et par parcelle. La date d'inventaire, au mois prés,est prise en compte dans les paramètres.
...
1. le seuil pour le calcul de l'indice journalier des tress hydrique (WS)

2. le fichier de donnée de circonférence issu de l'étape 1
...
# tableau des sites et couples d'inventaire à partir des mesures sélectionnées
ForestInvIsh<-datacr_sc %>% 
  select(Forest,CDate1,CDate2) %>% 
  distinct() %>% 
  transmute(Forest=Forest,CDate1=as.Date(CDate1,"%Y-%m-%d"),CDate2=as.Date(CDate2,"%Y-%m-%d")) %>% # Passe les Cdate en format date
...
# fonction de calcul de l'indice de stess hydrique, paramètre date début, date fin et donnees journalières de REW 
ISH<-function (date1, date2,Fplot, ISH_data) # ISHdata contient le calcul des ISH mensuel, la date du mois  et l'identifiants Forest
{
  sortie<-ISH_data %>% 
    filter(Forest==Fplot & (CDymIsh >= date1) & (CDymIsh < date2))  
    sum(sortie$Ishm)
    }

# Autre methode dplyr problème car si Y1 et Y2 sont des vecteur, car les vecteur logique ndoivent avoir la dimension de REWyearForest$year 
#Year1<-data15_1$Year1
#Year2<-data15_1$Year2
#Precipitation <- function(Y1 , Y2, For) { # Y1 et Y2 sont des entier
#  ApresDebut <- (REWyearForest$year > Y1 & REWyearForest$Forest==For)# Vecteur logique
#  AvantFin <- (REWyearForest$year <= Y2 & REWyearForest$Forest==For)# Vecteur logique
#  return(sapply(seq_along(Year1), function(i) sum(REWyearForest$Water_c[ (ApresDebut & AvantFin)[i] ])))
 
   # Pour chaque paire de Year1/Year2, on fait la somme, et on stocke le tout dans un vecteur de même taille que  Year1 et Year2
#}

#data15_2<-data15_1%>%
#  mutate(Water_2= Precipitation(Year1, Year2))


# boucle de calcul de l'indice pour chaque ligne foret_couple_inventaire
#temps_depart <-Sys.time()
i<-1
for(i in 1:nrow(ParacouInvIsh)) {
 # ISH(ParacouInvIsh$CDym1[i],ParacouInvIsh$CDym2[i],ParacouInvIsh$Forest[i],ISHdata)
  Fplot<-ParacouInvIsh$Forest[i]
  date1<-ParacouInvIsh$CDym1[i]
  date2<-ParacouInvIsh$CDym2[i]
  sortie<-ISHdata %>% filter(Forest==Fplot & (CDymIsh >= date1) & (CDymIsh < date2))  
  ParacouInvIsh$IshInv[i]<-sum(sortie$Ishm)
  }
...
## Ajout des varaibles climat au tableaux de données
...
datacr_cl<-datacr_sc %>%
  ...
  left_join(ParacouInvIsh,by=c("Forest","CDym1","CDym2"))
...
ParacouInvIsh_mo<-ParacouInvIsh %>% 
  rename(CDym3=CDym2) %>% 
  rename(CDym2=CDym1) 

datamo_cl<- datamo_cl_tmp %>% 
  left_join(ParacouInvIsh_mo,by=c("Forest","CDym2","CDym3")) %>% 
  rename(IshInvMo=IshInv) 
   
save(datacr_cl,datamo_cl,file=fich_clim_forest) # le 17/05/2019
...
#fich_sauv_clim<-"REW_paracou_78_17.Rdata" # fichier de sortie
#seuilREW<-0.4  # seuil pour calcul indice Water
#jour_inv<-273 # n° du jour d'inventaire de l'année 

#Chargement des données REW journalières de Paracou
# load("REW.1978.2013.Rdata") 
#REWdata<-read.csv2("Donnees_climat/REW_1978-2017_Fabien.csv",sep=",",dec=".")

# verif coherence annees meteo et donnees arbres
#sort(unique(tabMesuresSelect$CensusYear))
#sort(unique(REWdata$year)) # attention min(annee_climate) doit être égale à min(annee_inventaire) - 1
#temp<-tabMesuresSelect %>% filter(CensusYear==1978) # nb donnees perdues car annee 1978 sans indice de climat 

# à faire sous forme de fonction entre deux date d'inventaire pour l'adapter aux autres dispositifs
#Calcul indice annuel Paracou
#REWyear<-REWdata%>%
#  mutate(Waterj=seuilREW-REW)%>%   # calcul indice par jour
#  arrange(day_julian) %>%
#  mutate(Water=lag(Waterj,jour_inv))%>%  decalage indice journalier en fonction du jour d'inventaire : pour le calcul de l'indice de l'année 1996, on utilise les REW du 2 octobre 1995 au 1er octobre 1996
#  filter(Water > 0)%>%  #on ne garde que les jours avec un REW inférieur au seuil de 0.4"
#  group_by(year)%>%
#  summarise(Water=sum(Water))%>%   # calcul de l'indice annuel
#  filter(!is.na(Water))#%>%         # suppression année non complete (la premiere des donnees meteo)
 # mutate(Water_c=(Water-mean(Water)),Forest="Paracou") # on centre la variable climatique

#save(REWyear,file=fich_sauv_clim)
```



#3 Calcul des indices de perturbation pour les parcelles exploitées.
L'indice de perturbation est le pourcentage de biomasse enlevée ou morte, lors l'exploitation, par rapport à la biomasse initiale de la parcelle.
La valeur initiale est donnée par l'inventaire le plus récent avant exploitation, la valeur post exploitation est donnée par un inventaire quelques année après exploitation afin de prendre en compte l'effet retard de l'exploitation sur la mortalité.
...
la valeurde biomasse post exploitation sera donnée par l'inventaire post-exploitation, le plus proche de : année d'exploitation + YaLogg.

Une fois les année de mesures selectionnées, on fait le calcul sur les mesured corrrespondantes de Guyafor, en utilisant le package BIOMASS

Seuls les dispositifs de Paracou, Montagne Tortue et Manaré, ont de vrais inventaire pré-exploitation. 

Pour les parcelles Organabo et Risquetout le premier inventaire a eu lieu longtemps après l'exploitation. Sur ces parcelles exploitée, la moitiée des parcelles ont été éclaircies et d'autres sont restées en temoins, c'est à ditre exploitées mais pas eclaircies. D'après ONf, la parcelle Risquetout 8 a été très peu exploité (absence totale de souches). Nous tenterons de la prendre comme référence de biomasse pré-exploitation pour les parcelles de Risquetout.

## Par site, choix des années d'inventaire pré et post exploitation quand elles existent
...
dataDispoLogg<-read.csv2("foret-Campagne.csv") # extraction de la base Guyafor, contient les effectif de mesures par dates de fin d'inventaire et info sur type et année d'exploitation
...
## Calcul de l'indice par parcelle avec inventaire pré-exploitation
...
```{r perturbation_index, echo=TRUE}
#selection des mesure cibles pre et post exploitation
...
Taxo=correctTaxo(genus=as.character(liste_esp$Genus), species=as.character(liste_esp$Species)) # correct the species names
#WDdata=getWoodDensity(genus=Taxo$genusCorrected, species=Taxo$speciesCorrected, stand=Data$Plot) # get WD (mean and sd), l'option stand permet d'attribuer la WD moyenne pour les espèces inconnues, mais demeande de faire le calcule pour chaque arbres (sur table index_trait)
...
index_trait<-index_trait_mes %>% 
  ...
  filter(!is.na(Pre_CY)&!is.na(Post_CY)) %>% 
  # pour retirer les forêt de Risquetout et Organabo
  mutate(T_index=if_else((CensusYear==Pre_CY),"PreLogg","PostLogg")) %>%  
  # pour garder le type d'année
  group_by(ForestPlot,Surface,CensusYear,T_index) %>% 
  summarise(AGB_plot=sum(AGB)) %>%  
  # calcul de la biomasse par dispositif
  ungroup() %>%                                        
  # on dégroupe pour pouvoir supprimer l'année d'inventaire qui empecherait de faire le spread
  select(-CensusYear) %>% 
  spread(key=T_index, value=AGB_plot) %>%            
  # on crée des colonnes à partir de T_index et on y place les valeur de SurfTerr
  filter((!is.na(PreLogg))&(!is.na(PostLogg))) %>%   
  # on supprime les dispositifs avec une seule date
  mutate(IndexLogg=PostLogg/PreLogg*100/as.numeric(Surface))                 
  # calcul de l'indice à l'hectare

```

## Tentative d'ajout indice pour Risquetout et Organabo (non concluant)
...
index_rqt<-dispoGuyafor %>% 
 ...
 # filter((Plot=="P08 témoin") | (Plot=="P08 traitée")) %>% 
  ...
  select(-genus,-species) %>% 
  mutate(AGB=computeAGB(D=DBH, WD=meanWD, coord = cbind(Lon,Lat))) %>%
  group_by(ForestPlot,PlotSurface,CensusYear) %>%  
...
#knitr::kable(index_rqt, caption="BIommase Risquetout et Organabo")
...
```
Conclusion les AGB sur Risquetout et Organabo sont super variables dans le temps y compris sur les parcelles témoins et sur la parcelle 8 qui est la moins "exploitée". Leur utilisation dans le modèle est donc très délicate

## Ajout indice traitement et WD aux tableaux de données croissance et mortalité
...
save(nbmin_arbres,            # nombre minimal d'arbres par esp et par site
     ...
     file=fich_sauv_circ) #"Data16esp_s0_to_join.Rdata") #"S0" pour seuil à 0 pour le nb minimal d'arbres par esp et par site
