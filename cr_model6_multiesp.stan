...
//                        exp((-0.5)*pow(log(dbh_cr[n1]/(dbhmax_cr[n1]*Dopt+DoptC))/
//                            (Ks2*WD_cr[n1]*WD_cr[n1]+Ks*WD_cr[n1]+KsC),2)); 
...
// modele d'acroissement
  for(n1 in 1:Ncr){ 
    logacc_mu_cr[n1] = (oo_Gmax+
                        ...
                        (cr_clim+cr_Clesp[rgEsp_cr[n1]])*clim_cr[n1]+
                        (cr_logg+cr_Loesp[rgEsp_cr[n1]])*logg_cr[n1])*
                        exp((-0.5)*pow(log(dbh_cr[n1]/(dbhmax_cr[n1]*Dopt+DoptC))/(Ks2*WD_cr[n1]*WD_cr[n1]+Ks*WD_cr[n1]+KsC),2)); // obligation de faire un boucle sinon erreur de la fonction pow()
  }
//  for(n1 in 1:Ncr){ 
...
model {                            // Model block
  ...
  Ks2 ~ normal(0,100);         // priors
  KsC ~ normal(0,100);         // priors
  Dopt ~ normal(0,100);       // priors
  DoptC ~ normal(0,100);       // priors
